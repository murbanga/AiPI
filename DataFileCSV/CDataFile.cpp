...
static int CountCols(std::tistream& is, const TCHAR& delim)
{
	int nCols = 1;
	do
	{
...
{
public:
	AddTo(vector<vector<std::tstring> >& rTarget) : pvvStrTarget(&rTarget) {}
	AddTo(vector<vector<double> >& rTarget) : pvvTarget(&rTarget)    {}
	AddTo(vector<std::tstring>& rTarget)          : pvStrTarget(&rTarget)  {}
	AddTo(vector<double>& rTarget)          : pvTarget(&rTarget)     {}
	
	void operator()(const vector<std::tstring>& rSource)
		{ pvvStrTarget->push_back(rSource); }

	void operator()(const vector<double>& rSource)
		{ pvvTarget->push_back(rSource);    }

	void operator()(const std::tstring& rSource)
...
private:
	vector<vector<std::tstring> >* pvvStrTarget;
	vector<vector<double> >* pvvTarget;
	vector<std::tstring>*          pvStrTarget;
...
// they don't.
class ByName:
	public std::unary_function<std::tstring, bool>
{	
public:
	ByName(const TCHAR* szName) : m_szName(szName) {}

	bool operator()(const std::tstring& szVarName) const
	{
...
CDataFile::CDataFile(const TCHAR* szFilename, const int& dwReadFlags /* = DF::RF_READ_AS_DOUBLE */)
{
	m_delim       = DEFAULT_DELIMITER;
	m_dwReadFlags = dwReadFlags;
	m_reserve     = DEFAULT_RESERVE;
...
CDataFile CDataFile::FromVector(const TCHAR* szVariableName, const std::vector<double>& vData)
{
	CDataFile retDF;
...
{
	// clear data vectors
	vector<std::tstring>().swap(m_vstrVariableNames);
	vector<std::tstring>().swap(m_vstrSourceFilenames);
	vector<vector<double> >().swap(m_v2dData);
	vector<vector<std::tstring> >().swap(m_v2dStrData);
	...
	for(int iVar=firstVar; iVar<=lastVar; iVar++)
	{
		...
		if(dfs.pDataFile->m_dwReadFlags & RF_READ_AS_DOUBLE)
			...
			m_v2dStrData.push_back(vector<std::tstring>());
	}
...
int CDataFile::operator()(const int& iVariable, const int& iSample, TCHAR* lpStr)
{
	int retVal = 0;
	...
	if(m_dwReadFlags & RF_READ_AS_STRING)
	{
		try
		{
			_tcscpy(lpStr,m_v2dStrData.at(iVariable).at(iSample).c_str());
		...
		{	
#ifdef UNICODE
			m_szError = StringToWString(e.what());
#else
			m_szError = e.what();
#endif
			retVal = -1;
...
CDataFile CDataFile::FromVector(const TCHAR* szVariableName, const std::vector<std::tstring>& vData)
{
	...
	retDF.m_v2dStrData.push_back(vector<std::tstring>());
...
bool CDataFile::ReadFile(const TCHAR* szFilename, const unsigned& dwReadFlags)
{
...
try
{
#ifdef _UNICODE
		USES_CONVERSION;
		const char* aFilename = W2CA(szFilename);
#else
		const char* aFilename = szFilename;
#endif
		...
		std::tifstream inFile;
		
		inFile.open(aFilename);
		
		if(inFile.rdstate() & std::tios::failbit)
		{
			inFile.close();
...
std::tistream& operator>>(std::tistream& inFile, CDataFile& df)
{
	try
	{	
...
#ifdef _UNICODE	
			AfxMessageBox(_T("ERROR 0008: The Number of Headers is different than the Number of Data Columns!"));			
#else
		...
		for(iVar=0; iVar<nVars; iVar++)
		{
			...
			{
				df.m_v2dStrData.push_back(vector<std::tstring>());
		...
		do
		{
			for(iVar=0; iVar<nVars; iVar++)
			{
				...
				{
					if(df.m_dwReadFlags & RF_READ_AS_DOUBLE)
						df.m_v2dData.at(iVar+varOffset).push_back(_tcstod(buff, NULL));
		...
		if(df.m_vstrVariableNames.back().find(_T("\n"))!=-1)
	...
	{ 
#ifdef _UNICODE
		df.m_szError = df.StringToWString(e.what());
#else
		df.m_szError = e.what();
#endif
		throw e; 
...
#ifdef _UNICODE		
		AfxMessageBox(_T("ERROR 0000: UNKNOWN ERROR"));
#else
		throw exception(df.m_szError.c_str());
#endif
	}
...
std::tostream& operator << (std::tostream& outFile, const CDataFile& df)
{
	try
	{
		...
		for(iVar = 0; iVar<nVars; iVar++)
			outFile << df.m_vstrVariableNames.at(iVar).c_str() << ((iVar==nVars-1) ? _T("\n") : df.m_delim.c_str());
		...
		for(int iSamp = 0; iSamp<nSamps; iSamp++)
		{
			for(iVar = 0; iVar<nVars; iVar++)
			{
				...
				else if(df.m_dwReadFlags & RF_READ_AS_STRING)
				{
					...
					//try { WriteCSVstring(outFile, df.m_v2dStrData.at(iVar).at(iSamp).c_str()); }
					catch(...) {}
				}

				outFile << ((iVar==nVars-1) ? _T("\n") : df.m_delim.c_str());
			}
		}
	}
...
std::tostream& operator << (std::tostream& os, const DF::DF_SELECTION& dfs)
{
	try
	{
		int firstVar  = dfs.iVar[0]  < dfs.iVar[1]  ? dfs.iVar[0]  : dfs.iVar[1];
		...
		for(int iVar=firstVar; iVar<=lastVar; iVar++)
			os << dfs.pDataFile->m_vstrVariableNames.at(iVar).c_str() << ((iVar==lastVar) ? _T("\n") : dfs.pDataFile->m_delim.c_str());
...
bool CDataFile::SetData(const int&  iVariable, const int& iSample, const TCHAR* szValue)
{
	try
	{
...
bool CDataFile::AppendData(const int& iVariable, const TCHAR* szValue)
{
	try
	{
...
bool CDataFile::AppendData(const int& iVariable, const std::vector<std::tstring>& vszData)
{
	try
	{
		for_each
...
bool CDataFile::CreateVariable(const TCHAR* szName, const double& initial_value, const int& iSize)
{
	try
	{
		m_vstrSourceFilenames.push_back(_T(""));
...
// Attempts to create a variable with the name specified by szName with a size of 
// iSize and an initial_value. Returns true if successful, false if an error is encountered.
bool CDataFile::CreateVariable(const TCHAR* szName, const std::tstring& initial_value, const int& iSize)
{
	try
	{
		m_vstrSourceFilenames.push_back(_T(""));
		m_vstrVariableNames.push_back(szName);
		m_v2dStrData.push_back(vector<std::tstring>(iSize, initial_value));
		return true;
	}
...
bool CDataFile::CreateVariable(const TCHAR* szName, const std::vector<double>& vData)
{
	try
	{
		m_vstrSourceFilenames.push_back(_T(""));
...
bool CDataFile::CreateVariable(const TCHAR* szName, const std::vector<std::tstring>& vszData)
{
	try
	{
		m_vstrSourceFilenames.push_back(_T(""));
...
bool CDataFile::WriteFile(const TCHAR* szFilename, const TCHAR* szDelim)
{
	try
	{
...
int CDataFile::GetVariableName(const int& iVariable, TCHAR* lpStr)
{
	try
	{
		_tcscpy(lpStr, m_vstrVariableNames.at(iVariable).c_str());
...
int CDataFile::GetVariableName(const int& iVariable, std::tstring& rStr)
{
	try
	{
...
int CDataFile::GetLargestVectorSize_() const
{
	...
	{
		std::vector<std::vector<std::tstring> >::const_iterator it = m_v2dStrData.begin();
...
{
	m_szError = _T("");
	std::vector<std::tstring>().swap(m_vstrVariableNames);
	std::vector<std::tstring>().swap(m_vstrSourceFilenames);
	std::vector<std::vector<std::tstring> >().swap(m_v2dStrData);
...
int CDataFile::GetData(const int& iVariable, const int& iSample, std::tstring& rStr)
{
	if(m_dwReadFlags & RF_READ_AS_STRING)
	{
		try
...
// Returns the length of the string if successful. 
// Returns -1 if an error is encountered.
int CDataFile::GetData(const TCHAR* szVariableName, const int& iSample, std::tstring& rStr)
{
	int retVal = 0;
...
// Returns the length of the string if successful. 
// Returns -1 if an error is encountered.
int CDataFile::GetData(const int& iVariable, const int& iSample, TCHAR* lpStr)
{
	int retVal = 0;
...
// Returns the new size of rVector if successful.
// Returns -1 if an error is encountered.
int CDataFile::GetData(const int& iVariable, std::vector<std::tstring>& rVector)
{
	int retVal = 0;
...
// Returns the value at the specified location if successful.
// Returns DF::ERRORVALUE if an error is encountered.
double CDataFile::GetData(const TCHAR* szVariableName, const int& iSample)
{
	int index = LookupVariableIndex_(szVariableName);
...
int CDataFile::GetData(const TCHAR* szVariableName, const int& iSample, TCHAR* lpStr)
{
	int retVal = 0;
...
// Returns the new size of rVector if successful.
// Returns -1 if an error is encountered.
int CDataFile::GetData(const TCHAR* szVariableName, std::vector<double>& rVector)
{
	int index = LookupVariableIndex_(szVariableName);
...
// Returns the new size of rVector if successful.
// Returns -1 if an error is encountered.
int CDataFile::GetData(const TCHAR* szVariableName, std::vector<std::tstring>& rVector)
{
	int index = LookupVariableIndex_(szVariableName);
...
int CDataFile::GetVariableIndex(const TCHAR* szVariableName, const int& iStartingIndex /*=0*/)
{
...
// Returns -1 if the variable is not found.
int CDataFile::GetVariableIndex(const TCHAR* szVariableName, const TCHAR* szSourceFilename, const int& iStartingIndex /*=0*/)
{
	int offset = 0;
...
int CDataFile::LookupVariableIndex_(const TCHAR* szName,  const int& offset /*=0*/) const
{
	int retVal = 0;
...
//********************************************************
// Author:         Jack van Mook
// Function:      ReadCSVstring
// Description: Reads an string from an input stream conform CSV specification
// Updates:      23 jan 2005      -      Initial version
//********************************************************

int CDataFile::ReadCSVstring(std::tistream& inFile,		// input stream to pass
      TCHAR* buff,							// buffer to return the value
      int size,								// maximum buffersize passsed
      TCHAR delimiter						// what delimiter to be used
      )
{
      ...
      TCHAR cc;								// Current character
      TCHAR pc;								// Peek character
...
//else if (isspace(pc)) *** remove spaces after "???
			...
			//pMainFrame->m_wndStatusBar.SetBarProgress(1, TRUE, 100, cRead*100/size);
      }
      buff[bpos] = '\0';   // terminate string and return
      return cRead;
}
...
// Description: Adds "" in front and behind + escapes quote characters
// Updates:      23 jan 2005      -      Initial version
//********************************************************

int CDataFile::WriteCSVstring(
      std::tostream& outFile,		// output file
      const TCHAR *inS				// input string
      )
{
	  std::tstring outS;
      outS = inS;
...
std::wstring CDataFile::StringToWString(const std::string& s)
{
...
std::string CDataFile::WStringToString(const std::wstring& s)
{
