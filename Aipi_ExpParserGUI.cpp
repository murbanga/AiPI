// Aipi_ExpParserGUI.cpp: implementation of the CAipi_ExpParserGUIGUI class.
//
//////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////
CAipi_ExpParserGUI::CAipi_ExpParserGUI()
:m_query(GetSession())
{
	m_index = 0;
	//Data Base 
	...
	g_currentParserClass = EXP_PARSER_GUI;
...
void CAipi_ExpParserGUI::ExecCmdSearch(long pm, int i1, int i2)
{ 
	/*
	CString str;
...
CString CAipi_ExpParserGUI::LiteralVarSearch(long iform )
{
...
double CAipi_ExpParserGUI::NumericVarSearch(long iform)
{
...
///// Script /////
/////////////////////////////////////////////////////////////////////////////////
void CAipi_ExpParserGUI::ScriptSearch(long iform, tstring sId)
{
	...
	if( f != NOT_FOUND )
	{
		...
		if( lang == _T("JScript") || lang == _T("VBScript") )
		{
			...
			*/
			...
			{
				CAipi_Error err;
				err.displayGUIError(JSCRIPT_PARAM, JSCRIPT_ERROR, _T("The function parameters syntax is incorrect or not permited."));
				return;
			}
		}
		else if ( lang == _T("SQL") )
...
//This function saves the parameters values of the script function
//It return the index of the end of the function call (close parenthesis)
long CAipi_ExpParserGUI::ScriptSaveParam(long i1, long i2)
{
	CMainFrame* pMainFrame = (CMainFrame*)::AfxGetMainWnd();
	CAipi_Tokenizer tkz;
	int nParam = 0;
	...
	for( long i = i1; i< i2; ++i )
	{
		...
		*/
		if( pos >  posOpenParenthesis && pos < posCloseParenthesis )
		{
			...
			if( iForm != COMMA )
			{
				//Check if the parameters are numeric constant
				CString strParam = sToken.data();
				//AfxMessageBox(strParam);
				double result = isDigitOpt(strParam);
				if( result == 0 )
				{
					//AfxMessageBox(_T("No es digito"));
					//If the parameters are not numbers check if they are varaibles
					//CAipi_Symbol sym;
					//long idSymbol = sym.findId_IntForm(sToken);
					//if( idSymbol != NOT_FOUND )
					//{
						//AfxMessageBox(_T("Forma interna encontrado"));
						//If the variable numeric type was found get the current value
						CAipi_STableLocalNumeric st;
						double val = st.findIdValue(iForm);
						if( val != NOT_FOUND )
						{
							strParam.Format(_T("%f" ), val);
							//AfxMessageBox(strParam);
						}
						else
						{
							//If variable the literal type was found get the current value
							CAipi_STableLocalLiteral st;
							tstring val = st.findIdValue(iForm);
							if( val != _T("NOT_FOUND") )
							{
								strParam = val.data();
								//AfxMessageBox(strParam);	
							...
							else
							{
								////Error
								CAipi_Error err;
								err.displayGUIError(NO_VAR, SINTAX_ERROR, _T("Identifier [ " + strParam + " ] is not declare.") );
							}
						}
...
CString CAipi_ExpParserGUI::ScriptExecute(long pos1, long index1, int nParam, CString name, CString lang, CString code)
{
	m_ScriptObj.Reset();
...
void CAipi_ExpParserGUI::ScriptDisplayError(long nStartSel, int line, CString name)
{
	CMainFrame* pMainFrame = (CMainFrame*)::AfxGetMainWnd();
	//_bstr_t line = _T("");
	...
	//CString str;
	//str.Format(_T("start line %d"), nStartScriptLine);
	//AfxMessageBox(str);
...
//// SQL OleDB DataBase /////
///////////////////////////////////////////////////////////////////////////////////
int CAipi_ExpParserGUI::SqlSelectDB(tstring db)
{
...
void CAipi_ExpParserGUI::SqlShowDBDlg()
{
...
void CAipi_ExpParserGUI::SqlOpenConnectionDlg()
{
...
void CAipi_ExpParserGUI::SqlOpenOleDB()
{
...
void CAipi_ExpParserGUI::SqlRunQuery(CString query)
{
...
void CAipi_ExpParserGUI::SqlRunQueryVars(CString query, long index1, long index2)
{
...
bool CAipi_ExpParserGUI::isDigit(CString t)
{
	CString first = t[0];
...
double CAipi_ExpParserGUI::isDigitOpt(CString t)
{
...
bool CAipi_ExpParserGUI::isAlphabetic(CString t)
{
	t.MakeLower(); 
...
void CAipi_ExpParserGUI::initExpParserGUI()
{
...
void CAipi_ExpParserGUI::callExpParserGUI()
{
	getTokenizer();
	translation_unit();
}

void CAipi_ExpParserGUI::initTokenizer()
{
	CMainFrame* pMainFrame = (CMainFrame*)::AfxGetMainWnd();
	pMainFrame->m_wndStatusBar.SetBarProgress(1, TRUE, 0, 0);
	pMainFrame->m_wndStatusBar.SetUpBar(1, TRUE, TRUE);
...
void CAipi_ExpParserGUI::getToken()
{
...
long CAipi_ExpParserGUI::getTokenizer()
{
...
void CAipi_ExpParserGUI::printStack()
{
	CMainFrame* pMainFrame = (CMainFrame*)::AfxGetMainWnd();
...
void CAipi_ExpParserGUI::emit_declaration(int tk)
{
	...
	if ( tk == IDENTIFIER )
	{
		...
		tstring tk = pMainFrame->gvTokenizer[m_index-1].getToken();
		m_sCurrentId = tk;
		m_dCurrentId = id;
		...
		*/
		...
		if( m_currentType == AIPI_STRING || m_currentType == AIPI_CHAR || m_currentType == AIPI_SQLQUERY )
		{
...
void CAipi_ExpParserGUI::emit_expression(int tk)
{
	CAipi_Lexer lex;
...
void CAipi_ExpParserGUI::translation_unit()
{
	...
	if( m_lookahead != NUL )
	{
		...
		{
			//AfxMessageBox(_T("Entro a OUTPUT"));
			cmd_output();
			translation_unit();	
		}
		...
		{
			//AfxMessageBox(_T("Sale de ExpParserGUI"));
			//CString str;
...
/// Declaration Parser ////
//////////////////////////////////////////////////////////////////////////////////////////////////
void CAipi_ExpParserGUI::declaration_specifiers()
{
...
void CAipi_ExpParserGUI::declaration()
{
...
void CAipi_ExpParserGUI::declarator_list_init()
{
	declarator_init();
...
void CAipi_ExpParserGUI::declarator_init()
{
	declarator_direct();
...
void CAipi_ExpParserGUI::initializer()
{
...
void CAipi_ExpParserGUI::initializer_list()
{
...
void CAipi_ExpParserGUI::declarator_direct()
{
	...
	{
		//AfxMessageBox(_T("Open Parenthesis"));
		getTokenizer();
			...
			{
				//AfxMessageBox(_T("Declarator Direct"));
				declarator_direct();
			}
			if(		m_lookahead == AIPI_VOID 
				||	m_lookahead == AIPI_CHAR
			...
			{
				//AfxMessageBox(_T("Parameter List"));
				parameter_list();
			}
...
void CAipi_ExpParserGUI::parameter_declaration()
{
...
void CAipi_ExpParserGUI::parameter_list()
{
...
///// Expression Parser /////
/////////////////////////////////////////////////////////////////////////////////////////////
void CAipi_ExpParserGUI::expression_statement()
{
...
void CAipi_ExpParserGUI::expression()
{
...
void CAipi_ExpParserGUI::expression_assignment()
{
	/*
	if( m_lookahead == IDENTIFIER )
	{
...
void CAipi_ExpParserGUI::expression_logical_or()
{
...
void CAipi_ExpParserGUI::expression_logical_and()
{
	expression_equality();
...
void CAipi_ExpParserGUI::expression_equality()
{
...
void CAipi_ExpParserGUI::expression_relational()
{
...
void CAipi_ExpParserGUI::expression_aritmetic()
{
	expression_term();
...
void CAipi_ExpParserGUI::expression_term()
{
	expression_factor();
	while(true)
	{
...
void CAipi_ExpParserGUI::expression_unary_plus()
{
...
void CAipi_ExpParserGUI::expression_unary_minus()
{
...
void CAipi_ExpParserGUI::expression_unary_increment()
{
...
void CAipi_ExpParserGUI::expression_unary_decrement()
{
...
/*
void CAipi_ExpParserGUI::expression_factor()
{
...
void CAipi_ExpParserGUI::expression_atom_identifier()
{
...
void CAipi_ExpParserGUI::expression_argument_list()
{
	expression_assignment();
...
void CAipi_ExpParserGUI::expression_atom_constant()
{
	emit_expression(DIGIT);
	getTokenizer();
...
void CAipi_ExpParserGUI::expression_atom_literal()
{
	emit_expression(LITERAL);
	getTokenizer();
}
...
void CAipi_ExpParserGUI::type_name()
{
	//CString str;
...
void CAipi_ExpParserGUI::operator_unary()
{
	switch(m_lookahead)
	{
	case PLUS_SIGN:
...
void CAipi_ExpParserGUI::operator_assigment()
{
	switch(m_lookahead)
	{
	case EQUAL:
...
void CAipi_ExpParserGUI::cmd_db_open()
{
	getTokenizer();
...
void CAipi_ExpParserGUI::cmd_db_open_parameter()
{
...
void CAipi_ExpParserGUI::cmd_output()
{
	CMainFrame* pMainFrame = (CMainFrame*)::AfxGetMainWnd();
	getTokenizer();
	...
	if ( m_lookahead == LEFT_OP )
	{
		while( m_lookahead != SEMICOLON && m_lookahead != NUL)
		{
			...
			if( m_lookahead == LITERAL )
			{
				...
				ASSERT(pView);
				pView->GetRichEditCtrl().SetWindowText(strLit);
			...
			if( m_lookahead == IDENTIFIER )
			{
				...
				else
				{
					...
					CRichEditView* pView = (CRichEditView*) pChild->GetActiveView();
					ASSERT(pView);
					pView->GetRichEditCtrl().SetWindowText(strNum);
				}
...
int CAipi_ExpParserGUI::cmd_input()
{
	CMainFrame* pMainFrame = (CMainFrame*)::AfxGetMainWnd();
...
void CAipi_ExpParserGUI::cmd_input_continue()
{
	/*
...
void CAipi_ExpParserGUI::ctrl_jukebox()
{
...
void CAipi_ExpParserGUI::ctrl_mediaplayer()
{
...
void CAipi_ExpParserGUI::ctrl_imagefilter()
{
...
int CAipi_ExpParserGUI::ctrl_listoption()
{
...
void CAipi_ExpParserGUI::ctrl_listoption_continue()
{
	/*
	AfxMessageBox(_T("Input Val"));
	AfxMessageBox(g_currentCmdInput);
...
*/
	...
	//AfxMessageBox(_T("Out of Ctrl List Option Continue"));
...
void CAipi_ExpParserGUI::ctrl_oledb()
{
...
void CAipi_ExpParserGUI::cmd_db_open_parameter_list()
{
...
void CAipi_ExpParserGUI::cmd_db_close()
{
	getTokenizer();
