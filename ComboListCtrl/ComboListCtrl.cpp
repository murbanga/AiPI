...
CComboListCtrl::CComboListCtrl():m_blInited(FALSE)
{
...
CComboListCtrl::~CComboListCtrl()
{
	...
	CInPlaceEditCombo::DeleteInstance();  
}
...
CInPlaceEditCombo* CComboListCtrl::ShowInPlaceEdit(int iRowIndex, int iColumnIndex, CString& rstrCurSelection)
{
	// Create an in-place edit control
	CInPlaceEditCombo* pInPlaceEdit = CInPlaceEditCombo::GetInstance();
...
void CComboListCtrl::OnItemChanged(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	CString strAtribute = GetItemText(nSelected, 0);
	CString strType = GetItemText(nSelected, 1);
...
*/
/*
void CComboListCtrl::OnItemChanging(NMHDR* pNMHDR, LRESULT* pResult)
{
