...
int CCoordinates::GetExponent(double value)
{
	...
	if (value!=0)
	{
		if (fabs(value)>=1)
		{
			res = (int)(log(fabs(value))/log(10.00));
		} else
		{
			res = (-1)*(int)(fabs(log(fabs(value)))/log(10.00)+1);
		};
	};
	return res;
}
...
void CCoordinates::DivideInterval(int NMax, int* maxexp, double* Delta, int* num, double** scrpoints, double** wpoints)
{
	...
	double m_x1=p_v1*exp(-*maxexp*log(10.00));
	double m_x2=p_v2*exp(-*maxexp*log(10.00));
	while(fabs(m_x1-m_x2)<0.2f && m_x1<100 && m_x2<100)
	{
		*maxexp-=1;
		m_x1=p_v1*exp(-*maxexp*log(10.00));
		m_x2=p_v2*exp(-*maxexp*log(10.00));
	};
...
{
	double mv1, mv2;
	mv1 = p_v1*exp(-maxexp*log(10.00));
	mv2 = p_v2*exp(-maxexp*log(10.00));
	...
	{
		double v = CurrentValue*exp(maxexp*log(10.00));
		wp.push_back(v);
		CurrentX = WtoX(v);
