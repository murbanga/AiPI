...
int CGraphFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
    ...
    if (!pview_bar.Create(szBarName, this, size, TRUE, PVIEWBAR_ID))
    {
...
void CGraphFrame::AppendMenuItems(CMenu* menu)
{
	...
	{
	case LANG_SPANISH:
		menu->AppendMenu(MF_POPUP | MF_ENABLED | MF_STRING, (UINT)sub_menu.m_hMenu, _T("Ver"));
	break;
	default:
	...
	{
	case LANG_SPANISH:
		sub_menu.AppendMenu(menu_flags, GRAPH_RBMC_VIEW_POINT_WINDOW, _T("Puntos"));
	break;
	default:
