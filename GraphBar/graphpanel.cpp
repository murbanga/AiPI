...
void CGraphPanel::UpdateTitles(BOOL bXAxis, BOOL bRedraw)
{
    ...
    if (bXAxis)
    {
	    ...
	    hruler->SetNewTitles((TCHAR*)(LPCTSTR)grview->GetAxisProps(GRAPH_X_AXIS)->GetUOM(),
		    (TCHAR*)(LPCTSTR)str);
	    if (bRedraw) hruler->RedrawWindow();
    } else
    {
	    grview->GetAxisProps(GRAPH_Y_AXIS)->GetFullTitle(&str);
	    vruler->SetNewTitles((TCHAR*)(LPCTSTR)grview->GetAxisProps(GRAPH_Y_AXIS)->GetUOM(),
		    (TCHAR*)(LPCTSTR)str);
...
BOOL CGraphPropertyPage::OnInitDialog() 
{
...
/*
	...
	{
	case LANG_SPANISH:
		CWnd::SetWindowText(_T("General"));
		GetDlgItem(IDC_STATIC_GENERAL_COORDINATES)->SetWindowText(_T("Rango coordenadas"));
		GetDlgItem(IDC_GRAPH_POINTMARKS)->SetWindowText(_T("Marcas punto"));
		GetDlgItem(IDC_GRAPH_MOUSECOORDS)->SetWindowText(_T("Coordenadas ratón"));
		GetDlgItem(IDC_GEN_GRAPH_PROPS_AXIS)->SetWindowText(_T("Ejes"));
		break;
	default:
		CWnd::SetWindowText(_T("General"));
		GetDlgItem(IDC_STATIC_GENERAL_COORDINATES)->SetWindowText(_T("Coordinates range"));
		GetDlgItem(IDC_GRAPH_POINTMARKS)->SetWindowText(_T("Point marks"));
		GetDlgItem(IDC_GRAPH_MOUSECOORDS)->SetWindowText(_T("Mouse coordinates"));
		GetDlgItem(IDC_GEN_GRAPH_PROPS_AXIS)->SetWindowText(_T("Axis"));
		break;
	}
*/	
